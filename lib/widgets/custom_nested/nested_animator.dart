...
class MyCustomPainterAnimation extends StatefulWidget {
  const MyCustomPainterAnimation({Key? key}) : super(key: key);

  @override
  State<MyCustomPainterAnimation> createState() => _MyCustomPainterAnimationState();
}

class _MyCustomPainterAnimationState extends State<MyCustomPainterAnimation> {
  double _begin = 10.0;
  double _end = 0.0;
  bool _done = false;
  bool once = false;

  void _animateBlur() async {
    while (!_done) {
      if (!once) {
        Future.delayed(
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          IgnorePointer(
            // This is in case you want to tap things under the BackdropFilter
...
class WaveLayer extends CustomPainter {
  ...
  WaveLayer({@required required this.revealPercent, required this.primaryColor, required this.secondaryColor});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.cubicTo(maskWidth - waveHorRadius * 0.0509933125, curveStartY - waveVertRadius * 1.774752061, maskWidth,
        curveStartY - waveVertRadius * 1.8709256829, maskWidth, curveStartY - waveVertRadius * 2);
